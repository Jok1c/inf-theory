...
class DecimationCipher {
    ...
    validateKey(key) {
        ...
        if (Number.isNaN(k) || k < 1 || k > maxKey) {
            throw new Error(`Ключ для децимаций должен быть целым числом от 1 до ${maxKey} (введено: "${key.trim()}")`);
        }
...
class VigenereCipher {
    ...
    getKeyChar(keySequence, originalTextChars) {
    ...
    encrypt(text) {
        if (!text)
            return '';
        const keySequence = this.initialKey.split('');
        const result = [];
...
class CipherApp {
    ...
    constructor() {
        ...
        this.encryptTextBtn = document.getElementById('encryptTextBtn');
        this.decryptTextBtn = document.getElementById('decryptTextBtn');
